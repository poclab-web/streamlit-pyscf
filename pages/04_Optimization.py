"""
Streamlitを使用した幾何最適化アプリケーション

このスクリプトは、PySCFを使用して分子の幾何最適化を実行するための
インタラクティブなウェブインターフェースを提供します。ユーザーは分子の構造、
理論手法、基底関数系、および収束条件を入力し、幾何最適化計算を実行して
結果を可視化できます。

モジュール:
- `streamlit`: インタラクティブなウェブアプリケーションを作成するために使用。
- `matplotlib.pyplot`: エネルギー収束のプロットを作成するために使用。
- `logic.calculation`: 幾何最適化を実行するための `run_geometry_optimization` 関数と、
  理論手法および基底関数系の選択肢を提供。
